...
const ChecklistOutput = ({
  ...
  const containerStyle = config.disableDefaultStyle
    ? style.container
    : { ...checklistOutputStyle.container, ...style.container };
  const itemStyle = config.disableDefaultStyle
    ? style.item
    : { ...checklistOutputStyle.item, ...style.item };
  const checkboxStyle = config.disableDefaultStyle
    ? style.checkbox
    : { ...checklistOutputStyle.checkbox, ...style.checkbox };
  const labelStyle = config.disableDefaultStyle
    ? style.label
    : { ...checklistOutputStyle.label, ...style.label };
  ...
  return (
    <ul
      ...
      className={classnames("checklistUL", classNames.container)}
    >
      {content}
    </ul>
  );
